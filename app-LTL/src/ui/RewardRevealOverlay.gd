...
const EXCAVATION_BUILDUP_SFX_INTERVAL := 0.70
...
var _excavation_buildup_sfx_timer := 0.0
var _detail_fanfare_emitted := false
...
func _process(delta: float) -> void:
	...
	if current_step == "reveal_queue":
		_excavation_buildup_sfx_timer += delta
		if _excavation_buildup_sfx_timer >= EXCAVATION_BUILDUP_SFX_INTERVAL:
			_excavation_buildup_sfx_timer = 0.0
			interaction_sfx_requested.emit("excavation_buildup")
	timer += delta
...
func _finish_current_step_animation() -> void:
	...
	readable = true
	_emit_detail_fanfare_if_needed()
	queue_redraw()
...
func _emit_step_sfx(step: String) -> void:
	match step:
		"count_tease":
			interaction_sfx_requested.emit("reward_expectation")
		"count_lock":
			interaction_sfx_requested.emit("reward_count_fanfare")
		"reveal_queue":
			interaction_sfx_requested.emit("excavation_buildup")
func _emit_detail_fanfare_if_needed() -> void:
	if current_step != "reveal_queue" or _detail_fanfare_emitted:
		return
	_detail_fanfare_emitted = true
	interaction_sfx_requested.emit("excavation_detail_fanfare")
