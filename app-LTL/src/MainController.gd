...
const TERRAIN_SHIFT_SECONDS := 2.0
...
func _preview_options(seed_value: int) -> Dictionary:
	...
	return MainControllerRunFlowScript.preview_options(seed_value, leviathan, selected_leviathan_id, selected_start_color, selected_character_id, campaign_progress, growth_state.to_dict() if growth_state != null else {})
