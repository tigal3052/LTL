...
static func _complete_fusion_placement(controller, fusion_result: Dictionary) -> bool:
	var fused_artifact = fusion_result.get("artifact", null)
	if fused_artifact == null:
		return false
	if controller.held_from_rewards and controller.held_reward_index >= 0 and controller.held_reward_index < controller.local_rewards_list.size():
		var before_growth_for_fusion: Dictionary = controller.growth_state.to_dict()
		var claimed_fusion_reward_index: int = int(controller.held_reward_index)
		var fusion_reward_data = controller.local_rewards_list[claimed_fusion_reward_index]
		var fusion_reward_state = controller.preview_controller.run.apply_combat_input({
			"type": "claim_reward_effect",
			"reward": fusion_reward_data
		})
		var after_fusion_growth: Dictionary = fusion_reward_state.get("growth", {}).duplicate(true)
		controller.growth_state.from_dict(after_fusion_growth)
		controller.current_scene = fusion_reward_state
		...
		controller._emit_ui_telemetry(
			BuildRewardTelemetryScript.build_reward_selected(
				...
				{
					"inventory_diff": {
						"added": [str(fused_artifact.id)],
						"fused": [str(fusion_result.get("consumedArtifactId", "")), str(controller.held_artifact.id)]
					},
					"gold_delta": int(after_fusion_growth.get("gold", 0)) - int(before_growth_for_fusion.get("gold", 0)),
					"xp_delta": int(after_fusion_growth.get("xp", 0)) - int(before_growth_for_fusion.get("xp", 0))
				}
			)
		)
		controller.local_rewards_list.remove_at(claimed_fusion_reward_index)
		sync_reward_inspection_after_removal(controller, claimed_fusion_reward_index)
	...
	controller._append_localized_log("#a3be8c", "log.inventory.artifact_placed", [MainControllerDisplayTextScript.artifact_display_name(fused_artifact)])
	...
	controller.inspected_backpack_artifact = fused_artifact
	...
	controller.view.play_backpack_fusion_effect(fused_artifact)
	return true
...
static func place_held_artifact_at(controller, coord: Vector2) -> bool:
	...
	if controller.held_artifact == null:
		return false
	var exact_fusion_result: Dictionary = ItemFusionScript.try_fuse_exact_overlap(controller.inventory, controller.held_artifact, int(coord.x), int(coord.y))
	if bool(exact_fusion_result.get("ok", false)):
		return _complete_fusion_placement(controller, exact_fusion_result)
	if not controller.inventory.can_place_artifact(controller.held_artifact, int(coord.x), int(coord.y)):
		...
		if controller.view != null and controller.view.has_method("show_info_toast"):
			controller.view.show_info_toast(TextCatalogScript.t("log.inventory.invalid_placement"), 5.0)
		return false
	...
	if controller.view != null and controller.view.has_method("play_interaction_sfx"):
		controller.view.play_interaction_sfx("item_place")
...
static func sync_reward_inspection_after_removal(controller, removed_index: int) -> void:
	if controller.inspected_reward_index == removed_index:
		...
		controller.inspected_reward_index -= 1
static func request_discard_confirmation(controller, source: String) -> void:
	if controller.held_artifact == null:
		return
	controller.pending_discard_request = {
		"source": source,
		"rewardIndex": int(controller.held_reward_index),
		"origin": controller.held_inventory_origin,
		"artifactId": str(controller.held_artifact.id),
		"artifactName": MainControllerDisplayTextScript.artifact_display_name(controller.held_artifact)
	}
	if controller.view != null and controller.view.has_method("show_discard_confirmation"):
		controller.view.show_discard_confirmation(str(controller.pending_discard_request.get("artifactName", "")))
	elif controller.view != null and controller.view.has_method("set_confirm_overlay_visible"):
		controller.view.set_confirm_overlay_visible(true)
static func has_pending_discard(controller) -> bool:
	return controller.pending_discard_request is Dictionary and not controller.pending_discard_request.is_empty()
static func confirm_pending_discard(controller) -> bool:
	if not has_pending_discard(controller):
		return false
	var source := str(controller.pending_discard_request.get("source", ""))
	controller.pending_discard_request = {}
	if controller.view != null and controller.view.has_method("set_confirm_overlay_visible"):
	...
	if source == "backpack":
		...
		return true
	return false
static func cancel_pending_discard(controller) -> bool:
	if not has_pending_discard(controller):
		return false
	var source := str(controller.pending_discard_request.get("source", ""))
	controller.pending_discard_request = {}
...
static func on_backpack_slot_clicked(controller, coord: Vector2) -> void:
	...
	if slot_artifact != null:
		...
		controller.inspected_reward_index = -1
		controller.inspected_backpack_artifact = null
		controller.inventory.remove_artifact(controller.inventory.artifact_key(slot_artifact))
...
static func on_backpack_slot_drag_started(controller, coord: Vector2) -> void:
	...
	controller.inventory.remove_artifact(controller.inventory.artifact_key(dragged_artifact))
	controller._append_localized_log("#ffd766", "log.inventory.artifact_selected", [MainControllerDisplayTextScript.artifact_display_name(controller.held_artifact)])
	controller.view.update_backpack_ghost(controller.held_artifact)
	controller.view.render_backpack(controller.inventory)
	if controller.preview_controller.run != null:
...
static func on_backpack_slot_discard_requested(controller, _origin_coord: Vector2) -> void:
	if controller.held_artifact == null or controller.held_from_rewards:
		return
	request_discard_confirmation(controller, "backpack")
...
static func on_backpack_slot_hovered(controller, coord: Vector2) -> void:
	...
	var slot_id = str(controller.inventory.grid[int(coord.y)][int(coord.x)])
	if controller.held_artifact != null:
		if controller.held_from_rewards and not slot_id.is_empty() and controller.inventory.artifacts.has(slot_id):
			var target_artifact = controller.inventory.artifacts[slot_id]
			if int(target_artifact.x) != int(coord.x) or int(target_artifact.y) != int(coord.y):
				controller._clear_floating_tooltip()
				return
			var preview: Dictionary = ItemFusionScript.preview_duplicate_fusion(target_artifact, controller.held_artifact)
			if bool(preview.get("ok", false)):
				controller.view.show_fusion_tooltip(target_artifact, controller.held_artifact)
				return
		controller._clear_floating_tooltip()
		return
...
static func on_reward_meta_discard_requested_v2(controller, meta: Variant) -> void:
	...
	if controller.held_artifact == null or not controller.held_from_rewards or controller.held_reward_index != int(meta):
		return
	request_discard_confirmation(controller, "reward")
