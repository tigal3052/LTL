...
static func _node_snapshot(candidate: Dictionary) -> Dictionary:
	return {
		...
		"shieldMul": float(candidate.get("shieldMul", 1.0)),
		"healthMul": float(candidate.get("healthMul", 1.0)),
		"shieldMulByColor": candidate.get("shieldMulByColor", candidate.get("shield_mul_by_color", {})).duplicate(true) if candidate.get("shieldMulByColor", candidate.get("shield_mul_by_color", {})) is Dictionary else {},
		"healthMulByColor": candidate.get("healthMulByColor", candidate.get("health_mul_by_color", {})).duplicate(true) if candidate.get("healthMulByColor", candidate.get("health_mul_by_color", {})) is Dictionary else {},
