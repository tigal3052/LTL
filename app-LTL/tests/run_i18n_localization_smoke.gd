...
func _run() -> void:
	...
	_assert_eq(TextCatalogScript.t("action.start"), "전투 시작", "Korean start action resolves from catalog")
	TextCatalogScript.set_locale("en")
	_assert_eq(TextCatalogScript.t("action.start"), "Start Combat", "English start action resolves from catalog")
