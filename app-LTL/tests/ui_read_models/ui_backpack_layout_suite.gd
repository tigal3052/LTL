...
func test_main_scene_shared_layout_text_nodes_do_not_fit_content() -> void:
	...
	if main_instance == null:
		return
	var reward_summary = main_instance.get_node_or_null("RootMargin/AppShell/ActivePhaseContainer/RewardPanel/Margin/RewardBox/RewardBoardScroll/RewardBoard/RewardGrid/InspectorZone/Margin/ZoneBox/InspectorStage/InspectorSummary") as RichTextLabel
	var reward_name = main_instance.get_node_or_null("RootMargin/AppShell/ActivePhaseContainer/RewardPanel/Margin/RewardBox/RewardBoardScroll/RewardBoard/RewardGrid/InspectorZone/Margin/ZoneBox/InspectorStage/InspectorName") as Label
	var node_map_scene = NodeMapSceneScript.new()
	node_map_scene.render({
		"leviathan": {"name": "Ossuary Tortoise"},
		"selectedColor": "purple",
		"loadoutColors": ["red", "blue", "purple", "green"],
		"cards": [{
			"label": "Safe Scar",
			"selected": true,
			"weaknessLabel": "purple",
			"riskTier": "safe",
			"rewardBias": "baseline",
			"recommendedBuildHint": "Stable drill line",
			"finalStageDistance": 2
		}]
	})
	var node_select_detail = node_map_scene.find_child("DetailLabel", true, false) as RichTextLabel
	_assert(node_select_detail != null, "node-map scene exposes the node-select detail text view")
	...
	if node_select_detail != null:
		_assert_eq(bool(node_select_detail.fit_content), false, "node-select detail text does not content-fit inside the shared screen shell")
		_assert_eq(int(node_select_detail.autowrap_mode), int(TextServer.AUTOWRAP_WORD_SMART), "node-select detail text wraps inside the shared screen shell")
...
func test_main_scene_reward_tray_row_expands_to_keep_discard_zone_inside_panel() -> void:
	...
	var reward_board = main_instance.get_node_or_null("RootMargin/AppShell/ActivePhaseContainer/RewardPanel/Margin/RewardBox/RewardBoardScroll/RewardBoard") as VBoxContainer
	...
	var workspace_note = main_instance.get_node_or_null("RootMargin/AppShell/ActivePhaseContainer/RewardPanel/Margin/RewardBox/RewardBoardScroll/RewardBoard/RewardGrid/WorkspaceZone/Margin/ZoneBox/WorkspaceNote") as RichTextLabel
	...
	var bottom_row = main_instance.get_node_or_null("RootMargin/AppShell/ActivePhaseContainer/RewardPanel/Margin/RewardBox/RewardBoardScroll/RewardBoard/BottomRow") as HBoxContainer
	var discard_zone = main_instance.get_node_or_null("RootMargin/AppShell/ActivePhaseContainer/RewardPanel/Margin/RewardBox/RewardBoardScroll/RewardBoard/BottomRow/DiscardZone") as PanelContainer
	var confirm_zone = main_instance.get_node_or_null("RootMargin/AppShell/ActivePhaseContainer/RewardPanel/Margin/RewardBox/RewardBoardScroll/RewardBoard/BottomRow/ConfirmZone") as PanelContainer
	_assert(reward_box != null, "main scene exposes the reward tray box container")
	_assert(reward_board != null, "main scene exposes the reward claim board root container")
	_assert(reward_grid != null, "main scene exposes the reward claim board three-column layout")
	_assert(workspace_note != null, "main scene exposes the reward workspace note text surface")
	_assert(backpack_host != null, "main scene exposes the reward board backpack host")
	_assert(bottom_row != null, "main scene exposes the reward claim bottom row")
	_assert(discard_zone != null, "main scene exposes the reward tray discard zone")
	_assert(confirm_zone != null, "main scene exposes the inline claim confirmation zone")
	if reward_box != null:
