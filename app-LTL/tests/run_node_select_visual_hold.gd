...
func _run() -> void:
	...
	if MainScene == null:
		push_error("main scene failed to load for node-select visual hold")
		quit(1)
		return
	...
	if character_page == null:
		push_error("character select page missing for node-select visual hold")
		quit(1)
		return
	...
	if leviathan_page == null:
		push_error("leviathan select page missing for node-select visual hold")
		quit(1)
		return
