...
const DEFAULT_OUTPUT := "../docs/evidence/m6-screenshot-matrix/latest/character_select_1440x900.png"
...
func _run() -> void:
	...
	if MainScene == null:
		_fail("main scene failed to load for M6 visual capture")
		_finish(1)
		return
	var main_instance = MainScene.instantiate()
	if main_instance == null:
		_fail("main scene failed to instantiate for M6 visual capture")
		_finish(1)
		return
	...
	if save_error != OK:
		...
		_finish(1)
		return
	print("M6_INTERNAL_CAPTURED %s" % absolute_path)
	_finish(0)
...
func _drive_to_page(main_instance: Node, page_id: String) -> bool:
	match page_id:
		...
		_:
			_fail("unknown M6 visual capture page id: %s" % page_id)
			return false

func _go_to_leviathan_select(main_instance: Node) -> bool:
	await _advance_story_if_present(main_instance, "character_select")
	if not _expect_active(main_instance, "character_select", "character_select visual capture"):
		return false
	var character_page = main_instance.get("character_select_page")
	if character_page == null:
		_fail("character page missing before leviathan-select visual capture")
		return false
...
func _go_to_node_select(main_instance: Node) -> bool:
	...
	if leviathan_page == null:
		_fail("leviathan page missing before node-select visual capture")
		return false
...
func _go_to_battle(main_instance: Node) -> bool:
	if not await _go_to_node_select(main_instance):
		return false
	_press_current_node(main_instance, 0, "battle visual capture")
...
func _go_to_defeat(main_instance: Node) -> bool:
	...
	var controller = main_instance.get_node_or_null("MainController")
	if controller == null:
		_fail("controller missing before defeat visual capture")
		return false
...
func _advance_story_if_present(main_instance: Node, return_page_id: String) -> void:
	...
	if story_page == null or controller == null:
		_fail("story scene handoff missing before %s visual capture" % return_page_id)
		return
	var story: Dictionary = controller.get("active_story_scene")
	var scene_id := str(story.get("id", ""))
	if scene_id.is_empty():
		_fail("story scene id missing before %s visual capture" % return_page_id)
		return
	...
	await process_frame
	await process_frame
	_expect_active(main_instance, return_page_id, "%s story handoff" % return_page_id)

func _expect_active(main_instance: Node, expected: String, label: String) -> bool:
	var active := str(main_instance.get("active_page_id"))
	if active != expected:
		_fail("%s visual capture expected active page %s, got %s" % [label, expected, active])
		return false
	return true
...
func _resolve_output_path(raw: String) -> String:
	...
	return ProjectSettings.globalize_path("res://%s" % raw)
