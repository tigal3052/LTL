...
- `app-LTL/prototype/**`는 검증된 참조 구현이며 새 SoT로 승격하기 전까지 직접 수정하거나 정식 규칙으로 취급하지 않습니다.
...
- `app-LTL/prototype/**`는 검증된 프로토타입 보존 영역입니다. 새 기능 구현, 리팩터링, 주석 정제, 파일 이동을 직접 수행하지 않습니다. 필요한 경우 git 히스토리에서 복원하거나 별도 문서/정식 구현 경로에서 작업합니다.
